...
(function($) {
	...
	var Login = Base.klass.create({
		...
		events: {
			...
			'click .j-logo': 'clickLogo',

			'click .j-login-phone': 'clickLoginPhone'
		},
		init: function() {
			if (isWeixin) {
				this.elLoginWx.removeClass('hide');
			}else{
				this.elForm.removeClass('hide');
			}
		},
		confirm: function() {
			var self = this,
			...
			if (result) {
				switch (+result.errCode) {
					...
					case 0:
						...
						window.location.href = Base.url.param('sourceurl') ? decodeURIComponent(Base.url.param('sourceurl')) : 'user.html'; // 默认跳个人中心
						break;
		...
		clickLogo: function(){
			this.elForm.addClass('hide');
		},
		clickLoginPhone: function(){
			this.elForm.removeClass('hide');
		},
		chkExists: function() {
