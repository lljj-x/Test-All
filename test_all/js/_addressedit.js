...
(function($) {
	...
	var Address = Base.klass.create({
		...
		cgi: {
			one: '/api/addr/getOneAddr.jsp',
			update: '/api/addr/updateAddr.jsp',
			del: '/api/addr/removeOneAddr.jsp'
		},
		init: function() {
			this.aid = Base.url.param('id');
		...
		getOneBack: function(result) {
			...
			if (result && +result.errCode === 0) {
				this.renderOne(result.obj);
			} else {
				tip.show(result.errMsg);
			}

		},
		renderOne: function(o) {
			this.elName.val(o.receiver);
			this.elPhone.val(o.receiverPhone);

			var regionpathCache = o.regionpath.split(',').slice(1);

			this.cityFillCash = regionpathCache[1];
			this.areaFillCash = regionpathCache[2];

			this.elProvinceList.val(regionpathCache[0]).trigger('change');

			this.elAddress.val(o.receiverAddress);
			this.elPostCode.val(o.receiverZip);
		},
		...
		renderCityList: function(result) {
			...
			if (result) {
				...
				if (this.cityFillCash) {
					$city.val(this.cityFillCash).trigger('change');
					this.cityFillCash = null;
				}
			}
		},
		...
		renderAreaList: function(list) {
			...
			if (list && list.length) {
				...
				if (this.areaFillCash && areaCache[this.areaFillCash]) {
					$area.val(this.areaFillCash).trigger('change');
				}
			} else {
		...
		submit: function() {
			...
			if (can) {
				...
				$.post(this.cgi.update, params, this.proxy(this.submitBack));
			}
		},
