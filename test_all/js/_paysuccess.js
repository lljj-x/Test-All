...
(function($) {
	...
	var Paysuccess = Base.klass.create({
		...
		submit: function() {
			...
			can = can && this.chkIdcard();

			if (can) {
				params = {
					orderid: this.orderid,
					rname: this.elName.val(),
					idcard: this.elIdcard.val()
				}
		...
		submitBack: function(result) {
			...
			if (result) {
				if (+result.errCode === 0) {
					...
					gbid = this.cacheObj.gbid;

					tip.show('操作成功');

					setTimeout(function() {
						window.location.href = isGroupon ? '/groupon/grouponitem.html?gbid=' + gbid : 'user.html';
					}, 3000);
				} else {
					tip.show(result.errMsg);
				}
		...
		showSuccess: function() {
			...
			if (this.isGroupon()) {
				window.location.href = '/groupon/grouponitem.html?gbid=' + this.cacheObj.gbid;
			}
		},
