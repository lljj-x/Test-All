...
function checkWalletPas(form,callBack,callBackArr){
	form.validate({
	    ...
	    messages: {
	    	curPassword:{
	    		required:  jsLg.formMsg.password_required,
			...
			success:function(data){
				...
				if(data.status == 1){
					...
					$.layer({
						...
						area : ['auto','auto'],
						border : [0],
						offset : [($(window).height()-$(options.sucPopObj).height())/2 +'px', '50%'], 
	...
	if(setWalletPopBox.length){
		$("#js_setWalletPasBtn").on("click",function(){
			...
			walletPopIndex = GLOBAL.PopObj.openPop({
				border : [0],
				offset : [($(window).height()-popHeight)/2 + 'px', '50%'], 
				page : {html : html},
			...
			checkWalletPas($("#js_setWalletPassForm"),checkFormSuc,{url:"/m-users-a-set_wallet_password.htm",sucPopObj:"#js_setWalletSucPop"});
		});
...
(function(){//重置密码
	...
	if(setWalletPopBox.length){
		$("#js_resetWalletPasBtn").on("click",function(){
			...
			checkWalletPas($("#js_resetWalletPassForm"),checkFormSuc,{url:"/m-users-a-set_wallet_password.htm",sucPopObj:"#js_resetWalletSucPop"});
		});
...
(function(){//邮件认证
	$("#js_sendEmailBtn").on("click",function(){//发送邮件
		...
		if(!that.hasClass('isSend')){
			...
			$.get('/m-users-a-retrieve_wallet_password.htm?step=step1&send_email=1', function(data) {
