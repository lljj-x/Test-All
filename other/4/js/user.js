	...
	*/
	...
	$("#js_userNameText").hover(function(){
        ...
        if(offset && $(this).width() >= 280){
            $('body').append('<div class="tooltip-inner" style="position:absolute; height:20px;left:'+(offset.left )+'px;top:'+ (offset.top+5+ $(this).height()) +'px;' +' padding: 3px 8px; background-color:#fff; color:#333; border-radius:4px; border:1px solid #ccc;"><div style="position: absolute;width:0;height:0;border: 5px solid transparent; cursor: default; border-bottom-color: #999; border-top: none; left: 20px; top: -5px;"></div><div style="position: absolute;width:0;height:0;border: 5px solid transparent; cursor: default; border-bottom-color: #FFF; border-top: none; left: 20px; top:-4px;"></div>'+ $(this).data("text") +'</div>');
        }
	},function(){
	    $('body').find('.tooltip-inner').remove();
	})
	...
	//元素懒加载
	GLOBAL.lazyLoad.creatEle("textarea.js_slideReviews",scrollCallBackReviews);
	function scrollCallBackReviews(){
		$(".scrollHistory").flexslider({
			namespace:"",
			animation: "slide",
			selector: ".slides > li",
...
//		});
	...
	$('#js_gcPay').on("click",function(){
		$("#js_PaypalDown").find("form").submit();
	...
	$('.js_deletePro').click(function(){
		if (confirm(jsLg.user.confirmDelete)) {
			...
			url = $list.data('url');
			deleteProAjax(url, goodsId, $listObj);
		}
	});
	...
	function deleteProAjax(url,goodsId, $listObj){
		$.ajax({
			type: "POST",
			url: url,
			data: "objid=" + goodsId,
        ...
        $.ajax({
           ...
           success: function(msg){
             if(!msg.status){
                ...
                rsendEmailIndx = $.layer({
                    type : 1,
                    area : ['345px','auto'],
                    title : jsLg.sendEmaix,
                    move : ['.xubox_title' , true],
                    shade : [0.2 , '#000' , true],
                    border : [5 , 0.5 , '#333', true],
                    page : {dom : '.resendEmmailPop'},
                    close : function(index){
                        layer.close(index);
                    }
                });
...
(function(){
	...
	$(".js_showtrackInfo").click(function(){
		...
		if($this.data("isShow") != 1){
			...
			$.get('/fun/index.php?act=get_tracking',{trackno:$(this).attr("data-trackno")}, function(data) {
				/*optional stuff to do after success */
				if(data.status && data.list){
					html += '<ul>';
					$.each(data.list, function(index, val) {
						/* iterate through array or object */
						html += '<li><span class="mr15">'+ val.ondate +'</span>'+ val.status +' <strong class="ml15 fb">'+ val.area +'</strong></li>';
					});

					html += '</ul>';

				}else{
					html = '<p style="font-size:20px; text-align: center; margin:50px 0;">'+ "Sorry, your shipping information has not updated yet" +'</p>';
				}

				$targetBox.slideDown().find('dd').html(html);
...
(function(){
    ...
    window.GLOBAL.popAddCart = {
        ...
        ini:function(){
            ...
            that.wrap.on("keyup","#js_pop_quantity_input",function(e){//直接填写
                var Jquantity = $(this);
            ...
            that.wrap.on("click","#js_popAddTopCart",function(e){//点击购买按钮       
                ...
                var Jthat = $(this);
                var goodsId = Jthat.data("goods_id"),
                    num = $("#js_Popinput_quantity").val();
                ...
                Jthat.addClass('uping');
                $.getJSON(URL, function(data) {
                	Jthat.removeClass('uping');

                    var  msg = data.ms;             
                    if (msg.indexOf('Added To Cart')>0){//加入购物车成功
                        (that.openPopIndx != null) && GLOBAL.PopObj.closePop(that.openPopIndx);
            ...
            $(".js_addCart").click(function() {
                ...
                that.openPopIndx = GLOBAL.PopObj.openPop({border:[0],offset : ['auto' , '50%'],page : {dom:'#js_pop_addcart'}});
...
// iframe在ie下会导致父元素失去hover，所以使用mouseenter和mouseleave代替css的:hover
(function() {
    var timeoutId = null;
     $('.u-share').on('mouseenter',function() {
        $(this).addClass('hover');
     })
     .on('mouseleave',function() {
        ...
        // {begin} google+ 无法编写评论bug 处理
        function docMouseMoveHandler(e) {
            if ($(e.target).parents("table.gc-bubbleDefault").length == 0 && $(e.target).parents(".u-share").length == 0) {
                self.removeClass('hover');
                $(document).off("mousemove", docMouseMoveHandler);
            }
        }
        // google+ 评论层 处理
        $("table.gc-bubbleDefault").parent().one("mouseenter", function() {
            timeoutId && clearTimeout(timeoutId);
            $(document).on("mousemove", docMouseMoveHandler);
        });
        // {end} google+ 无法编写评论bug 处理
     });
})();