    ...
    jQuery.fn.imageMaps = function(setting) {
		...
		$container.each(function(){
			...
			$images.each(function(){
				...
				var _img_conrainer = $("#contents").parent();
				_img_conrainer.append('<div class="button-conrainer"><input type="button" value="添加" /></div>').append('<div class="link-conrainer"></div>').append($.browser.msie ? $('<div class="position-conrainer" style="position:absolute"></div>').css({
					background:'#fff',
					opacity:0
				...
				if(map_name !=''){
					...
					container.find('map[name='+map_name+']').find('area[shape=rect]').each(function(){
						...
						_link_conrainer.append('<p ref="'+index+'" class="map-link"><span class="link-number-text">SKU '+index+'</span>: <input type="text" size="60" name="sku[]" value="'+$(this).attr('href')+'" /><input type="hidden" class="rect-value" name="rect[]" value="'+coords+'" /></p>');
						coords = coords.split(',');
		...
		$('.button-conrainer input[type=button]').click(function(){
			...
			image = (image == '' ? '' : '['+ image + ']');
			_link_conrainer.append('<p ref="'+index+'" class="map-link"><span class="link-number-text">SKU '+index+' </span>: <input type="text" size="60" name="sku[]" value="" /><input type="hidden" class="rect-value" name="rect[]" value="10,10,100,40" /></p>');
