	...
	FocusOne : function(obj){
		with(obj){
			...
			try{
				...
				span.innerHTML = "<img src=/temp/skin1/images/onFocus.gif>"+obj.getAttribute("msg");
			}
				...
				case "SafeString" :
					if(!eval(this[_dataType])) {
						...
						try{
							...
							span.innerHTML = "<img src=/temp/skin1/images/onCorrect.gif>符合要求.";
						}
						catch(e){}	//alert(e.description);
					}
					break;
			...
			switch(mode){
				...
				case 3 :
					try{
						...
						obj.parentNode.appendChild(span);
						span.innerHTML = this.ErrorMessage[1].replace(/\d+:/,"<img src=/temp/skin1/images/onError.gif>");
					}
					catch(e){}//alert(e.description);
					//obj.focus();
					break;
	...
	Validate : function(theForm, mode){
		...
		for(var i=0;i<count;i++){
			with(obj.elements[i]){
				...
				var _CheckChina = getAttribute("CheckChina");				
				//if(typeof(_CheckChina) == "object" || typeof(this[_CheckChina]) == "undefined")
					//continue;
				
					if(_CheckChina == 'yes'){ 
					    if (isChina(value)){
							var tarobj = getAttribute("msg").replace('请输入','');
							this.AddError(i, tarobj+'含中文字符，请检查');
						}
					}
					...
					case 3 :
						for(var i=1;i<errCount;i++){
							try{
								...
								span.innerHTML = this.ErrorMessage[i].replace(/\d+:/,"<img src=/temp/skin1/images/onError.gif>");
							}
							catch(e){}//alert(e.description);
						}
						this.ErrorItem[1].focus();
