(function(){
    ...
    function otherPayNum(){
        ...
        $(".js_alsoNeedPayNum").attr("orgp",allPrice-walletPay);
        //根据cookie，改变币种
    ...
    //计算总数
    function jisuan_total(selfUserWallet){
        //积分换算
       ...
       // free_sub_total = (free_sub_total == undefined)?0:parseFloat(free_sub_total);
        ...
        $(".shipping_method").each(function(){
            ...
            if($(this).prop("checked")){
                shippingOrgp = $("#sm"+$(this).val()).attr("orgp");
                $("#shipping_sub_total").attr("orgp",shippingOrgp);
                return false;
            }
        })
        ...
        //var cheknum = ($needTrakNumber.attr("checked"))?parseFloat($needTrakNumber.val()):0;
    ...
    // .on("change",".paymentselect",function(event){
    //     event.preventDefault();
    //     if($(this).prop("checked")){
    //         $(this).prop("checked",false);
    //     }
    //     setChoicePayment($(this).closest('dl'));
    //     return false;
    // }).on("change",".iswallet",function(event){
    //     event.preventDefault();
    //     var $this = $(this);
    ...
    $("#theForm").submit(function(){
        //alert( checkInfo())
       $("#js_upFormBtn").prop("disabled",true).addClass('uping');

        if(checkInfo()){
            return true;
        }else{
            $("#js_upFormBtn").prop("disabled",false).removeClass('uping');
            return false;
        }
        return false;
    });
