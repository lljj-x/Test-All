...
(function(){
	...
	// function showHideSechNav(){
	// 	var $cateListNum = $("#js_dp_seach_navList").find('li'),$btn = $("#see_all_department");
	//     var numLi =  $cateListNum.length;
	    


	//     if(numLi > 13){
	//     	var $gtMoreLi = $cateListNum.filter(":gt(13)");
	//         $gtMoreLi.hide();
	       
	//         $btn.html(jsLg.seeAllDepartments).show().find('span').text(numLi-13);
	...
	//         		$gtMoreLi.show();
	//         		$btn.html(jsLg.seeFewerDepartments).find('span').text(numLi-13);
	//         		$(this).data("isShow",1);
	//         	}else{
	//         		$gtMoreLi.hide();
	//         		$btn.html(jsLg.seeAllDepartments).find('span').text(numLi-13);
	//         		$(this).data("isShow",0);
...
(function(){
	...
	RelatedKey.prototype.ini=function(){
		if(this.link.length > 3){
			...
			this.contains.append('<i data-flag="0" class="fl">'+ jsLg.More +'</i>');
			this._operal();
		}
	}
...
(function(){
	//新品页面分类js
	$("#js_newCateList,#js_free_shippList").on("click",function(e){
		var Jtarget = $(e.target);
		
		if(Jtarget.is("p")){//点击一级分类

			var Jparent = Jtarget.parent("div.item");
			var Jsibling = Jparent.siblings('div.item');

			Jparent.addClass('cur');
			Jsibling.removeClass('cur').find('.openCur').removeClass('openCur');


		}else if(Jtarget.is("dt")|| Jtarget.is("dt i")){//点击二级分类
			var Jparent = Jtarget.closest("dl");
			if(Jparent.find('dd li').length){
				if(Jparent.hasClass('openCur')){
					Jparent.removeClass('openCur');
				}else{
					Jparent.addClass('openCur').siblings('dl').removeClass('openCur');
				}
	...
	// //freeshipping 分类页面js
	// $("#js_free_shippList").on("click",".triangle",function(e){
	// 	var targetWrap = $(this).closest('.item');

	// 	targetWrap.addClass('on').siblings('div.item').removeClass('on');
	// });
